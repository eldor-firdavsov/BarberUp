 ...
 * Pre-fills phone from Telegram bot registration — user does NOT type it here.
 */
...
export default function Onboarding() {
    ...
    async function loadTelegramData() {
        ...
        if (tgUser?.first_name) setName(tgUser.first_name);
        ...
        if (!record?.phone) {
            setError(t('onboarding.botFirst'));
        }
        setLoading(false);
    }
    ...
    async function handleSubmit() {
        ...
        try {
            if (role === 'barber') {
                ...
                const { data, error: err } = await supabase.from('barbers').insert([{
                    fullname: name.trim(),
                    phone,
                    telegram_id: tgUser?.id ?? null,
                }]).select().single();

                if (err) { setError(t('errors.generic')); setSaving(false); return; }

                // Update telegram_users with role
                if (tgUser?.id) {
                    await supabase
                        .from('telegram_users')
                        .update({ role: 'barber' })
    ...
    return (
        <div className="min-h-screen bg-[#f5f5f7] flex flex-col px-6 pt-12 pb-8">
            {/* Logo */}
            <div className="flex flex-col items-center mb-10">
                ...
                <h1 className="text-2xl font-extrabold text-[#111] tracking-tight">BarberUp</h1>
                <p className="text-sm text-[#666] mt-1">{t('onboarding.title')}</p>
            </div>
            ...
            <div className="bg-white rounded-[28px] border border-black/5 shadow-[0_20px_60px_rgba(0,0,0,0.07)] p-7 space-y-5">
                ...
                <div>
                    ...
                    <div className="w-full px-4 py-3.5 rounded-2xl bg-[#f8f8f8] border border-black/5
                        text-sm text-[#666] font-medium">
                ...
                <div>
                    ...
                    <input
                        ...
                        className="w-full px-4 py-3.5 rounded-2xl border border-black/5 text-sm
                            outline-none focus:border-[#378ADD] transition bg-white"
                    />
                </div>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            ...
                            className={`py-5 rounded-2xl border-2 flex flex-col items-center gap-2
                        ...
                        >
                            ...
                            <span className={`text-sm font-bold
                                ${role === 'client' ? 'text-[#378ADD]' : 'text-[#111]'}`}>
                                {t('onboarding.roleClient')}
                            </span>
                            <span className="text-xs text-[#666]">{t('onboarding.roleClientDesc')}</span>
                        </button>
                        ...
                        >
                            ...
                            <span className={`text-sm font-bold
                                ${role === 'barber' ? 'text-[#378ADD]' : 'text-[#111]'}`}>
                                {t('onboarding.roleBarber')}
                            </span>
                            <span className="text-xs text-[#666]">{t('onboarding.roleBarberDesc')}</span>
                        </button>
                    </div>
                </div>
                ...
                <button
                    ...
                    className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#378ADD] to-[#185FA5] text-white text-base font-bold
                        disabled:opacity-40 active:scale-[0.98] transition shadow-[0_10px_25px_rgba(55,138,221,0.28)]"
                >
